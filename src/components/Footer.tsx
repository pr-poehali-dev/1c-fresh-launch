export default function Footer() {
  return (
    <footer className="bg-[#2b2b2b] text-white py-12">
      <div className="max-w-[1980px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {/* Logo and Company Info */}
          <div className="flex flex-col gap-4">
            <img 
              src="https://cdn.poehali.dev/files/9cdb632b-9883-4574-85fb-43d2feb4beac.png" 
              alt="ЦАВ - Центр Автоматизации и Внедрений" 
              className="h-12 w-auto"
            />
            <div className="text-sm text-gray-400">
              <p className="font-semibold text-white mb-2">ООО "МОЛОТОВ ТРАСТ"</p>
              <p>Официальный партнер 1C Fresh</p>
              <p>Код № 56678-57</p>
            </div>
          </div>

          {/* Company Details */}
          <div className="text-sm text-gray-400">
            <h3 className="font-semibold text-white mb-3">Реквизиты</h3>
            <div className="space-y-1">
              <p>ИНН: 5906060110</p>
              <p>КПП: 590401001</p>
              <p>ОКПО: 73907860</p>
              <p className="mt-3">Директор: Иваницкая Е. С.</p>
            </div>
          </div>

          {/* Contacts and Social */}
          <div className="text-sm text-gray-400">
            <h3 className="font-semibold text-white mb-3">Контакты</h3>
            <div className="space-y-2 mb-4">
              <p>Телефон: <a href="tel:+73422700001" className="text-white hover:text-orange-500 transition-colors">+7 (342) 270-00-01</a></p>
              <p>Адрес: 614007, Пермский край,<br/>г. Пермь, ул. Революции, д. 14, кв. 57</p>
            </div>
            
            {/* Social Icons */}
            <div className="flex items-center gap-3 mt-4">
              <a 
                href="https://t.me/your_channel" 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-8 h-8 bg-white rounded flex items-center justify-center hover:bg-gray-200 transition-colors"
              >
                <svg className="w-4 h-4 text-[#2b2b2b]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.161c-.18 1.897-.962 6.502-1.359 8.627-.168.9-.5 1.201-.82 1.23-.697.064-1.226-.461-1.901-.903-1.056-.692-1.653-1.123-2.678-1.799-1.185-.781-.417-1.21.258-1.911.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.139-5.062 3.345-.479.329-.913.489-1.302.481-.428-.008-1.252-.241-1.865-.44-.752-.244-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.831-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635.099-.002.321.023.465.141.121.099.154.232.17.326.016.094.036.308.02.476z"/>
                </svg>
              </a>
              <a 
                href="https://wa.me/73422700001" 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-8 h-8 bg-white rounded flex items-center justify-center hover:bg-gray-200 transition-colors"
              >
                <svg className="w-4 h-4 text-[#2b2b2b]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.77-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86s.274.072.376-.043c.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.045.072.045.419-.1.824zm-3.423-14.416c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm.029 18.88c-1.161 0-2.305-.292-3.318-.844l-3.677.964.984-3.595c-.607-1.052-.927-2.246-.926-3.468.001-3.825 3.113-6.937 6.937-6.937 1.856.001 3.598.723 4.907 2.034 1.31 1.311 2.031 3.054 2.03 4.908-.001 3.825-3.113 6.938-6.937 6.938z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Copyright */}
        <div className="border-t border-gray-700 pt-6 text-center text-gray-400 text-sm">
          © 2025 ООО "МОЛОТОВ ТРАСТ". Все права защищены.
        </div>
      </div>
    </footer>
  );
}